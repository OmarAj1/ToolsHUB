...
export function LocalHashGenerator() {
  ...
  const HashItem = ({ title, value, type, isLoading = false }: { title: string, value: string, type: string, isLoading?: boolean }) => (
    <div className="bg-slate-800 bg-stone-900 border border-stone-200 border-stone-800 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/50 transition-shadow">
      <div className="bg-stone-50 bg-stone-950 px-4 py-2 border-b border-stone-200 border-stone-800 flex justify-between items-center">
        <h3 className="text-sm font-bold text-stone-700 text-stone-300">{title}</h3>
        <button 
          onClick={() => handleCopy(value, type)}
          disabled={!value || isLoading}
          className="text-stone-400 hover:text-stone-700 hover:text-stone-200 transition-colors disabled:opacity-50"
          title="Copy Hash"
        >
          {copied === type ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4 text-purple-500" />}
        </button>
      </div>
      <div className="p-4 relative min-h-[4rem] flex flex-col justify-center">
        {isLoading ? (
          ...
          <code className="text-sm md:text-base font-mono break-all text-stone-600 text-stone-400">
            {value || <span className="text-stone-300 text-stone-700">Waiting for input...</span>}
          </code>
        )}
      </div>
    </div>
      ...
      <div className="w-full max-w-4xl space-y-6">
        <div className="mb-6 flex items-center space-x-4">
          <div className="w-12 h-12 rounded-xl bg-blue-100 bg-blue-900/30 flex items-center justify-center">
            <Fingerprint className="w-6 h-6 text-blue-600 text-blue-400" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-stone-900 text-white">Local Hash Generator</h2>
            <p className="text-stone-500">100% Offline. Generates instantly in your browser.</p>
          </div>
        </div>

        <div className="bg-slate-800 bg-stone-950 rounded-2xl border border-stone-200 border-stone-800 focus-within:ring-2 focus-within:ring-blue-500/50 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-stone-200 border-stone-800 bg-stone-50 bg-stone-900 flex justify-between items-center">
          ...
          <textarea 
            ...
            className="w-full h-32 md:h-40 p-5 bg-transparent resize-none focus:outline-none text-stone-200 text-lg"
          />
        </div>
