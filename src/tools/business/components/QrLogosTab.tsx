...
export function QrLogosTab({ state, setters, handlers }: any) {
  ...
  return (
     <div className="max-w-2xl animate-in fade-in duration-300 space-y-8">
       ...
       <div className="w-full bg-[#FAFAFA] dark:bg-[#1A1A1A] border border-slate-100 dark:border-[#2A2A2A] p-6 rounded-3xl shadow-sm">
         <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
           <Globe className="w-5 h-5 text-indigo-500" /> Auto-Fetch Logo from URL
         </h3>
         <div className="flex gap-3">
           <div className="relative flex-1">
             ...
             <input
               ...
               className="w-full pl-11 pr-4 py-3 bg-white dark:bg-[#111111] border border-slate-200 dark:border-[#333] rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all dark:text-white"
             />
           </div>
           <button
             onClick={fetchLogo}
             disabled={!logoUrl || isLoading}
             className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
           >
             {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Fetch Base Logo'}
           </button>
         </div>
       ...
       <div className="flex flex-col sm:flex-row gap-6 items-start">
         {!logoImage ? (
           <label className="flex flex-col items-center justify-center w-full sm:w-40 h-40 border-2 border-dashed border-slate-300 dark:border-slate-700 bg-[#FAFAFA] dark:bg-[#161616] rounded-3xl hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-white dark:hover:bg-[#1A1A1A] cursor-pointer transition-all hover:shadow-md">
             ...
             <span className="text-xs text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-widest mt-2">Upload</span>
