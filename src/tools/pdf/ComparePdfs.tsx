...
function ComparePdfsBase() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 text-slate-50 text-slate-50 h-[calc(100vh-80px)] flex flex-col">
       <div className="text-center mb-6 shrink-0">
          <h2 className="font-bold flex items-center justify-center gap-2 text-xl">
             <Columns className="w-6 h-6 text-blue-500" /> Basic Side-by-Side PDF Viewer
          </h2>
          <p className="text-sm text-slate-400">Fast local side-by-side rendering using your browser's native PDF engine.</p>
       </div>

       <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
          {/* File 1 */}
          <div className="bg-slate-800 bg-slate-800 border border-slate-700 border-slate-700 rounded-2xl p-4 shadow-sm flex flex-col h-full">
             <div className="flex justify-between items-center mb-4 shrink-0">
               <h3 className="font-bold text-sm">Version A</h3>
               <label className="cursor-pointer bg-slate-800 hover:bg-slate-200 bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded text-xs font-bold transition-colors flex items-center gap-1">
                 <Upload className="w-3.5 h-3.5" /> Upload File
                 <input type="file" accept="application/pdf" className="hidden" onChange={handleUpload1} />
               </label>
             </div>
             
             <div className="flex-1 bg-slate-900 bg-slate-900 border border-slate-700 border-slate-700 rounded-lg overflow-hidden relative">
                {file1 ? (
...
export const ComparePdfs = withPdfSafeBoundary(ComparePdfsBase);
