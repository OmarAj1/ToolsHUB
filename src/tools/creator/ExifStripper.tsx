...
export function ExifStripper() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 text-slate-800 dark:text-slate-200">
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-8 flex gap-3 text-sm">
      ...
      {!cleanedImage ? (
        <label className="border-2 border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-2xl h-80 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
          ...
          <span className="text-slate-500">Supports JPG, PNG, WebP</span>
          
          {isProcessing && (
            <div className="mt-6 flex items-center justify-center gap-2 text-indigo-500 font-bold">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-sm text-center">
            ...
            <p className="text-slate-500 mb-8 max-w-md mx-auto">All unseen metadata (EXIF) has been successfully scrubbed from <strong>{originalName}</strong>.</p>
            
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a 
                href={cleanedImage} 
                download={`cleaned_${originalName}`}
                className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-xl transition-colors"
              >
                <Download className="w-5 h-5" /> Download Safe Image
              </a>
              <button 
                onClick={handleReset}
                className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 font-bold py-3 px-8 rounded-xl transition-colors"
              >
                Clean Another
              </button>
            </div>
