...
export function ThumbnailSafeZone() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 text-slate-800 dark:text-slate-200">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl mb-8 flex flex-wrap gap-6 items-center justify-between shadow-sm">
         <div>
           <h2 className="font-bold mb-1">YouTube Thumbnail Safe Zones</h2>
           <p className="text-sm text-slate-500">Preview how UI elements cover your thumbnail across devices.</p>
         </div>

         <div className="flex gap-4">
           <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
             <input type="checkbox" checked={showDesktop} onChange={e => setShowDesktop(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4" />
             <Monitor className="w-4 h-4 text-slate-400" /> Desktop Overlays
           </label>
           <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
             <input type="checkbox" checked={showMobile} onChange={e => setShowMobile(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500 w-4 h-4" />
             <Smartphone className="w-4 h-4 text-slate-400" /> Mobile Overlays
           </label>
         </div>
         
         <label className="cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg transition-colors flex items-center gap-2">
      ...
      <div className="relative w-full aspect-video bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm flex items-center justify-center">
