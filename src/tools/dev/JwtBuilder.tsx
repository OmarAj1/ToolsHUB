        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl">
             ...
             <textarea 
               ...
               className="w-full bg-slate-50 dark:bg-slate-950 font-mono text-sm p-3 rounded h-32 text-red-600 dark:text-red-400 focus:outline-none"
             />
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl">
             ...
             <textarea 
               ...
               className="w-full bg-slate-50 dark:bg-slate-950 font-mono text-sm p-3 rounded h-48 text-purple-600 dark:text-purple-400 focus:outline-none"
             />
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl">
             ...
             <input 
               ...
               className="w-full bg-slate-50 dark:bg-slate-950 font-mono text-sm p-3 rounded text-blue-600 dark:text-blue-400 focus:outline-none"
             />
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl flex flex-col h-full min-h-[400px]">
           <h3 className="font-bold mb-4">Generated JWT</h3>
           {error ? (
              <div className="flex-1 flex items-center justify-center text-red-500 font-bold bg-red-50 dark:bg-red-900/10 rounded-lg">
                {error}
              </div>
           ) : (
              <div className="flex-1 w-full bg-slate-50 dark:bg-slate-950 rounded-lg p-6 font-mono text-lg break-all whitespace-pre-wrap leading-relaxed shadow-inner">
                {jwt && (
                  <>
                    <span className="text-red-600 dark:text-red-400 font-bold">{jwt.split('.')[0]}</span>
                    <span className="text-slate-400">.</span>
                    <span className="text-purple-600 dark:text-purple-400 font-bold">{jwt.split('.')[1]}</span>
                    <span className="text-slate-400">.</span>
                    <span className="text-blue-600 dark:text-blue-400 font-bold">{jwt.split('.')[2]}</span>
                  </>
                )}
              </div>
           )}
